	...
	$.ajax({
		...
		success: function(result, status, xhr){
			...
			if (printer.type === "M806"){
				...
				setTrayLevel(printer.hostName, $(result).find('#deviceStatusPage').children()[2].childNodes[2].childNodes[1].childNodes[4].childNodes[3].childNodes[2].textContent, "test", 2);
				setTrayLevel(printer.hostName, $(result).find('#deviceStatusPage').children()[2].childNodes[2].childNodes[1].childNodes[6].childNodes[3].childNodes[2].textContent,"test", 3);
...
var setTrayLevel = function(hostName, trayLevel, paperType, trayNumber) {
	$('#' +hostName + '-tray' + trayNumber + '-type').text(paperType);
	var imgLevel = -1;
